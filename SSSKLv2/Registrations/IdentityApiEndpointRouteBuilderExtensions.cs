...
public static class IdentityApiEndpointRouteBuilderExtensions
{
    ...
    /// </summary>
    /// <typeparam name="ApplicationUser">The type describing the user. This should match the generic parameter in <see cref="UserManager{TUser}"/>.</typeparam>
    /// <param name="endpoints">
    /// The <see cref="IEndpointRouteBuilder"/> to add the identity endpoints to.
    /// Call <see cref="EndpointRouteBuilderExtensions.MapGroup(IEndpointRouteBuilder, string)"/> to add a prefix to all the endpoints.
    /// </param>
    /// <returns>An <see cref="IEndpointConventionBuilder"/> to further customize the added endpoints.</returns>
    public static IEndpointConventionBuilder MapSssklIdentityApi(this IEndpointRouteBuilder endpoints)
    {
            ...
            ([FromBody] RegisterRequest registration, HttpContext context, [FromServices] IServiceProvider sp) =>
        {
            ...
            {
                throw new NotSupportedException($"{nameof(MapSssklIdentityApi)} requires a user store with email support.");
            }
        ...
        // POST /logout: sign the user out. If using cookie authentication this will clear the authentication cookie.
        routeGroup.MapPost("/logout", async Task<Results<Ok, EmptyHttpResult>>
            ([FromQuery] bool? useCookies, [FromServices] IServiceProvider sp) =>
        {
            var signInManager = sp.GetRequiredService<SignInManager<ApplicationUser>>();

            // Ensure we sign out using the application cookie scheme when cookies are used.
            if (useCookies == true)
            {
        ...
        .Add(endpointBuilder =>
        {
            ...
            confirmEmailEndpointName = $"{nameof(MapSssklIdentityApi)}-{finalPattern}";
